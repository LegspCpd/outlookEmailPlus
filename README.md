...
| `D1_DATABASE_NAME` | D1 数据库名称 | 创建 D1 时指定的名称（如 `outlook-email-db`） |
| `KV_NAMESPACE_ID` | KV 命名空间 ID | `wrangler kv:namespace list` 输出中的 `id` |
| `KV_BINDING_NAME` | KV 绑定变量名 | 默认 `SESSION_KV`（与 wrangler.toml 一致即可） |
| `R2_BUCKET_NAME` | R2 存储桶名称 | 你创建的 R2 存储桶名 |
